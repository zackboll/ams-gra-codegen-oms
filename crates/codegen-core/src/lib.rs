...
/// formed at all -- an open-world abstract value, a cyclic dependency, invalid
/// IR. There is then *no* emitted surface, so there are no generated names to
/// judge. Naming must not manufacture a verdict from output that can never
/// exist: the semantic failure is owned by the backend's own diagnostics and
/// must remain the authoritative error.
pub(crate) enum NamePreflightPlan<'a> {
    Planned(Vec<TypeEmission<'a>>),
    UnavailableBecauseSemanticFailure,
}

/// Form the shared emission plan for generated-name analysis, once per
/// schema/world.
///
...
pub(crate) fn name_preflight_plan(
    ...
    match plan_type_emissions(schema, world) {
        ...
        Err(_) => NamePreflightPlan::UnavailableBecauseSemanticFailure,
    }
}
