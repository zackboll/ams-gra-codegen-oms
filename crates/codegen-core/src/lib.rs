...
/// The schema-owned declarations whose names actually reach the generated
/// top-level scope, in Schema IR order.
///
/// This is the single source of truth shared by generated-name preflight and
/// by capability attribution, derived from the same [`plan_type_emissions`]
/// the backends consume. It exists so naming can ask "is this name emitted?"
/// without duplicating the planner's world policy or the renderers' skip
/// rules.
///
/// `None` means the plan could not be formed at all -- an open-world abstract
/// value, a cyclic dependency, invalid IR. That is a *semantic* failure owned
/// by the backend's own diagnostics, and naming must not convert it into a
/// name verdict, so callers fall back to the conservative whole-schema view
/// rather than silently reserving nothing.
pub(crate) fn emitted_top_level_declaration_names(
    schema: &SchemaIr,
