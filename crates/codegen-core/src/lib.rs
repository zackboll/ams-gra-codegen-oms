...
pub use backend_names::{
    ...
    backend_names_are_renderable, generated_enum_variant_name, schema_emits_ada_binary_vectors,
...
pub use string_profile::{
    NATO_SPECIAL_WORDS_MAX_LENGTH, NATO_SPECIAL_WORDS_MIN_LENGTH, NATO_SPECIAL_WORDS_PATTERN,
    NATO_SPECIAL_WORDS_PREFIX, NATO_SPECIAL_WORDS_SUFFIX_MAX_LENGTH,
    NATO_SPECIAL_WORDS_SUFFIX_MIN_LENGTH, StringProfile, StringProfileError,
    UCI_SCHEMA_VERSION_MAX_LENGTH, UCI_SCHEMA_VERSION_MIN_LENGTH, UCI_SCHEMA_VERSION_PATTERN,
    UCI_UUID_LENGTH, UCI_UUID_NIL, UCI_UUID_PATTERN, VISIBLE_ASCII_MAX_CODE_POINT,
    VISIBLE_ASCII_MIN_CODE_POINT, WHITESPACE_VISIBLE_CARRIAGE_RETURN, WHITESPACE_VISIBLE_LINE_FEED,
...
impl TypeEmission<'_> {
    /// Whether this planned entity actually carries its schema-owned name as a
    /// generated top-level declaration.
    ///
    /// A planned `Declaration` is not automatically a generated declaration.
    /// Every backend renders an **abstract Record** purely as inheritance
    /// metadata -- `backend-ada`, `backend-rust`, and `backend-cpp` all return
    /// early from their `TypeKind::Record` arm when `is_abstract` is set -- so
    /// its effective fields are folded into each concrete descendant and no
    /// host-language type is ever written for the base itself.
    ///
    /// An abstract **Choice** is deliberately *not* treated the same way. No
    /// backend skips it, so when the planner hands one back as a
    /// `Declaration` it really is emitted (Ada additionally emits its
    /// `_Kind` companion). Collapsing both structural kinds into one
    /// "abstract is never emitted" rule would stop reserving a name that
    /// genuinely appears in the output and let a real collision through.
    ///
    ...
    pub(crate) fn emits_own_top_level_name(&self) -> bool {
        match self {
            ...
            Self::AbstractValue(_) => true,
        }
    }
}

/// The generated-name surfaces for one schema/world, or the marker that no
/// surface exists to analyse.
///
/// Generated-name analysis operates on **actual emitted entities**, never on
/// raw Schema IR declarations. This type is how that fact is carried to the
/// naming layer: `Planned` holds exactly the [`TypeEmission`]s the backends
/// render, so top-level names, member regions, and Ada helper stems are all
/// derived from one model rather than from two that can disagree.
///
/// `UnavailableBecauseSemanticFailure` means the shared plan could not be
/// formed as a whole -- an open-world abstract value, a cyclic dependency,
/// invalid IR. The planner fails **closed and globally**: one unrepresentable
/// target aborts the entire plan. Naming must not convert that into a name
/// verdict, so the affected surfaces are reported as unavailable and the
/// semantic failure stays the authoritative error.
///
/// The variant still carries the surfaces that are *individually* well-formed,
/// because a global planning abort is not evidence that every other
/// declaration lacks an emitted shape. Discarding them would silently stop
/// checking names the backends really do emit, which is a far larger
/// correctness hole than the one this boundary exists to close.
pub(crate) enum NamePreflightPlan<'a> {
    Planned(Vec<TypeEmission<'a>>),
    UnavailableBecauseSemanticFailure(Vec<TypeEmission<'a>>),
}

impl<'a> NamePreflightPlan<'a> {
    /// The emitted surfaces to analyse, whichever way the plan was formed.
    pub(crate) fn surfaces(&self) -> &[TypeEmission<'a>] {
        match self {
            Self::Planned(emissions) | Self::UnavailableBecauseSemanticFailure(emissions) => {
                emissions
            }
        }
    }
...
/// Form the shared emission surfaces for generated-name analysis, once per
/// schema/world.
///
/// Callers hold the result for the whole of one validation or attribution
/// pass, so [`plan_type_emissions`] is never re-run inside a per-declaration
/// loop.
///
/// When the shared planner succeeds its result is used verbatim. When it fails
/// the surfaces are rebuilt locally by applying the *renderers'* own emission
/// rules per declaration, without the planner's global fail-closed behaviour:
/// ordering and cross-entity dependency resolution are irrelevant to naming,
/// and a target that cannot be represented simply contributes no surface --
/// which is exactly the truth naming needs.
pub(crate) fn name_preflight_plan(
    schema: &SchemaIr,
    world: GenerationWorld,
) -> NamePreflightPlan<'_> {
    match plan_type_emissions(schema, world) {
        Ok(emissions) => NamePreflightPlan::Planned(emissions),
        Err(_) => {
            NamePreflightPlan::UnavailableBecauseSemanticFailure(emission_surfaces(schema, world))
        }
    }
}
